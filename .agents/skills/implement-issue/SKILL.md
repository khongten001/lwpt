...
6. If validation fails or requirements are ambiguous, stop and ask.
7. Present implementation options with tradeoffs, a verification plan, and a recommendation. Do not code until the user picks one or asks you to proceed with the recommendation.
8. Branch / worktree:
...
9. Implement the smallest complete change that satisfies the chosen approach.
...
14. Invoke `/create-pr` to commit, push, and open the draft pull request. Include `Closes #<issue>` in the PR body.
